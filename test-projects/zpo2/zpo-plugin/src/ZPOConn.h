...
namespace packet_analysis::BR_INF_UFRGS_ZPO {

class ZPOSessionAdapter;

class ZPOAnalyzer final : public IP::IPBasedAnalyzer {
public:
    ZPOAnalyzer();
    ~ZPOAnalyzer() override = default;

    static zeek::packet_analysis::AnalyzerPtr Instantiate() {
        return std::make_shared<ZPOAnalyzer>();
    }

    packet_analysis::IP::SessionAdapter *MakeSessionAdapter(
...
private:
    ...
    void Echo(double t, const struct icmp *icmpp, int len, int caplen,
              const u_char *&data, const IP_Hdr *ip_hdr,
              ICMPSessionAdapter *adapter);
    void Redirect(double t, const struct icmp *icmpp, int len, int caplen,
                  const u_char *&data, const IP_Hdr *ip_hdr,
                  ICMPSessionAdapter *adapter);
    void RouterAdvert(double t, const struct icmp *icmpp, int len, int caplen,
                      const u_char *&data, const IP_Hdr *ip_hdr,
                      ICMPSessionAdapter *adapter);
    ...
    void NeighborSolicit(double t, const struct icmp *icmpp, int len,
                         ...
                         ICMPSessionAdapter *adapter);
    void RouterSolicit(double t, const struct icmp *icmpp, int len, int caplen,
                       const u_char *&data, const IP_Hdr *ip_hdr,
                       ICMPSessionAdapter *adapter);

    RecordValPtr BuildInfo(const struct icmp *icmpp, int len, bool icmpv6,
                           const IP_Hdr *ip_hdr);
