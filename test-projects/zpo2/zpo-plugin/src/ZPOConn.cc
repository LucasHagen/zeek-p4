...
ZPOAnalyzer::ZPOAnalyzer()
    : IPBasedAnalyzer("ZPO_CONN", TRANSPORT_UNKNOWN, PORT_SPACE_MASK, false) {}

SessionAdapter* ZPOAnalyzer::MakeSessionAdapter(Connection* conn) {
    auto* root = new ZPOSessionAdapter(conn);
    root->SetParent(this);
...
bool ZPOAnalyzer::BuildConnTuple(size_t len, const uint8_t* data,
                                 Packet* packet, ConnTuple& tuple) {
    if (!CheckHeaderTrunc(sizeof(event_t), len, packet)) return false;

    const event_t* event_hdr = (const event_t*)data;

    // This analizer is only used for IP based protocols
    // IPv6 not yet supported
    if (htons(event_hdr->protocol_l3) != ETH_P_IP) return false;

    tuple.src_addr = IPAddr(ip(event_hdr->src_addr));
    tuple.dst_addr = IPAddr(ip(event_hdr->dst_addr));
    tuple.src_port = htons(event_hdr->src_port);
    tuple.dst_port = htons(event_hdr->dst_port);
    uint8_t t_proto = htons(event_hdr->protocol_l4);
    ...
    if (packet->proto == IPPROTO_ICMP)
        tuple.dst_port = htons(ICMP4_counterpart(
            icmpp->icmp_type, icmpp->icmp_code, tuple.is_one_way));
    else if (packet->proto == IPPROTO_ICMPV6)
        tuple.dst_port = htons(ICMP6_counterpart(
            icmpp->icmp_type, icmpp->icmp_code, tuple.is_one_way));
    else
        reporter->InternalError(
            "Reached ICMP packet analyzer with unknown packet protocol %x",
            packet->proto);

    return true;
}

void ZPOAnalyzer::DeliverPacket(Connection* c, double t, bool is_orig,
                                int remaining, Packet* pkt) {
    auto* adapter = static_cast<ICMPSessionAdapter*>(c->GetSessionAdapter());

    const u_char* data = pkt->ip_hdr->Payload();
    int len = pkt->ip_hdr->PayloadLen();
    // If segment offloading or similar is enabled, the payload len will return
    // 0. Thus, let's ignore that case.
    ...
    if (!zeek::detail::ignore_checksums &&
        ...
        switch (ip->NextProto()) {
            ...
            default:
                reporter->Error("unexpected IP proto in ICMP analyzer: %d",
                                ip->NextProto());
                return;
        }
    ...
    if (ip->NextProto() == IPPROTO_ICMP)
        NextICMP4(run_state::current_timestamp, icmpp, len, remaining, data,
                  ip.get(), adapter);
    else if (ip->NextProto() == IPPROTO_ICMPV6)
        NextICMP6(run_state::current_timestamp, icmpp, len, remaining, data,
                  ip.get(), adapter);
    else {
        reporter->Error("expected ICMP as IP packet's protocol, got %d",
                        ip->NextProto());
        return;
    }

    // Store the session in the packet in case we get an encapsulation here. We
    // need it for handling those properly.
    pkt->session = c;
...
void ZPOAnalyzer::NextICMP4(double t, const struct icmp* icmpp, int len,
...
void ZPOAnalyzer::NextICMP6(double t, const struct icmp* icmpp, int len,
                            int caplen, const u_char*& data,
                            const IP_Hdr* ip_hdr, ICMPSessionAdapter* adapter) {
    switch (icmpp->icmp_type) {
        // Echo types.
...
void ZPOAnalyzer::ICMP_Sent(const struct icmp* icmpp, int len, int caplen,
...
zeek::RecordValPtr ZPOAnalyzer::BuildInfo(const struct icmp* icmpp, int len,
...
TransportProto ZPOAnalyzer::GetContextProtocol(const IP_Hdr* ip_hdr,
                                               uint32_t* src_port,
                                               uint32_t* dst_port) {
    ...
    switch (proto) {
        case TRANSPORT_ICMP: {
            ...
            *src_port = ntohs(icmpp->icmp_type);
            ...
            if (ip4)
                *dst_port = ntohs(ICMP4_counterpart(
                    icmpp->icmp_type, icmpp->icmp_code, is_one_way));
            else
                *dst_port = ntohs(ICMP6_counterpart(
                    icmpp->icmp_type, icmpp->icmp_code, is_one_way));
...
zeek::RecordValPtr ZPOAnalyzer::ExtractICMP4Context(int len,
                                                    const u_char*& data) {
    ...
    if (len < (int)sizeof(struct ip) || ip_hdr_len > uint32_t(len)) {
        ...
        bad_checksum = !run_state::current_pkt->l3_checksummed &&
                       (detail::in_cksum(
                            reinterpret_cast<const uint8_t*>(ip_hdr->IP4_Hdr()),
                            ip_hdr_len) != 0xffff);
...
zeek::RecordValPtr ZPOAnalyzer::ExtractICMP6Context(int len,
                                                    const u_char*& data) {
...
void ZPOAnalyzer::Echo(double t, const struct icmp* icmpp, int len, int caplen,
    ...
    if (ip_hdr->NextProto() == IPPROTO_ICMPV6)
        f = (icmpp->icmp_type == ICMP6_ECHO_REQUEST) ? icmp_echo_request
                                                     : icmp_echo_reply;
    else
        f = (icmpp->icmp_type == ICMP_ECHO) ? icmp_echo_request
                                            : icmp_echo_reply;
    ...
    adapter->EnqueueConnEvent(
        ...
        val_mgr->Count(iid), val_mgr->Count(iseq),
        make_intrusive<StringVal>(payload));
}

void ZPOAnalyzer::RouterAdvert(double t, const struct icmp* icmpp, int len,
                               int caplen, const u_char*& data,
                               const IP_Hdr* ip_hdr,
                               ICMPSessionAdapter* adapter) {
...
void ZPOAnalyzer::NeighborAdvert(double t, const struct icmp* icmpp, int len,
                                 int caplen, const u_char*& data,
                                 const IP_Hdr* ip_hdr,
                                 ICMPSessionAdapter* adapter) {
...
void ZPOAnalyzer::NeighborSolicit(double t, const struct icmp* icmpp, int len,
                                  int caplen, const u_char*& data,
                                  const IP_Hdr* ip_hdr,
                                  ICMPSessionAdapter* adapter) {
...
void ZPOAnalyzer::Redirect(double t, const struct icmp* icmpp, int len,
                           int caplen, const u_char*& data,
                           const IP_Hdr* ip_hdr, ICMPSessionAdapter* adapter) {
...
void ZPOAnalyzer::RouterSolicit(double t, const struct icmp* icmpp, int len,
                                int caplen, const u_char*& data,
                                const IP_Hdr* ip_hdr,
                                ICMPSessionAdapter* adapter) {
    ...
    adapter->EnqueueConnEvent(f, adapter->ConnVal(),
                              BuildInfo(icmpp, len, 1, ip_hdr),
                              BuildNDOptionsVal(caplen, data, adapter));
}

void ZPOAnalyzer::Context4(double t, const struct icmp* icmpp, int len,
                           int caplen, const u_char*& data,
                           const IP_Hdr* ip_hdr, ICMPSessionAdapter* adapter) {
    EventHandlerPtr f = nullptr;
...
void ZPOAnalyzer::Context6(double t, const struct icmp* icmpp, int len,
                           int caplen, const u_char*& data,
                           const IP_Hdr* ip_hdr, ICMPSessionAdapter* adapter) {
    EventHandlerPtr f = nullptr;
...
zeek::VectorValPtr ZPOAnalyzer::BuildNDOptionsVal(int caplen,
                                                  const u_char* data,
                                                  ICMPSessionAdapter* adapter) {
    static auto icmp6_nd_option_type =
        id::find_type<RecordType>("icmp6_nd_option");
    static auto icmp6_nd_prefix_info_type =
        id::find_type<RecordType>("icmp6_nd_prefix_info");
    ...
    while (caplen > 0) {
        ...
        switch (type) {
            ...
            case 3:
                ...
                {
                    if (caplen >= 30) {
                        auto info = make_intrusive<zeek::RecordVal>(
                            icmp6_nd_prefix_info_type);
                        uint8_t prefix_len = *((const uint8_t*)(data));
                        bool L_flag =
                            (*((const uint8_t*)(data + 1)) & 0x80) != 0;
                        bool A_flag =
                            (*((const uint8_t*)(data + 1)) & 0x40) != 0;
                        ...
                        info->Assign(3,
                                     make_intrusive<IntervalVal>(
                                         (double)ntohl(valid_life), Seconds));
                        info->Assign(4,
                                     make_intrusive<IntervalVal>(
                                         (double)ntohl(prefer_life), Seconds));
                        info->Assign(5,
                                     make_intrusive<AddrVal>(IPAddr(prefix)));
            ...
            case 5:
                ...
                {
                    if (caplen >= 6)
                        rv->Assign(5, val_mgr->Count(ntohl(
        ...
        if (set_payload_field) {
            String* payload =
                new String(data, std::min((int)length, caplen), false);
