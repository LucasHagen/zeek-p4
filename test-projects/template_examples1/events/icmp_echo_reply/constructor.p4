...
if(hdr.ipv4.isValid()) {
    hdr.icmp_echo_reply_event.info.v6 = (z_bool) 8w0x00;
    hdr.icmp_echo_reply_event.info.len = (z_count) hdr.ipv4.total_len - hdr.ipv4.minSizeInBytes() - hdr.icmp.minSizeInBytes();
    hdr.icmp_echo_reply_event.info.ttl = (z_count) hdr.ipv4.ttl;
...
if(hdr.ipv6.isValid()) {
    hdr.icmp_echo_reply_event.info.v6 = (z_bool) 8w0xFF;
    hdr.icmp_echo_reply_event.info.len = (z_count) hdr.ipv6.total_len - hdr.ipv6.minSizeInBytes() - hdr.icmp.minSizeInBytes();
    hdr.icmp_echo_reply_event.info.ttl = (z_count) hdr.ipv6.hop_limit;
