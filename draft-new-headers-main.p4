    ...
    apply {
        if (standard_metadata.instance_type == INSTANCE_TYPE_NORMAL) {
            ...
            if (hdr.arp_ipv4.isValid()) {
                if(hdr.arp.opcode == 1) {
                    meta.event_type = ZPO_ARP_REQUEST_EVENT_UID;
                } else if(hdr.arp.opcode == 2) {
                    meta.event_type = ZPO_ARP_REPLY_EVENT_UID;
                }
            }
            if (hdr.icmp.isValid()) {
                if(hdr.icmp.type_ == ICMP_ECHOREPLY) {
                    meta.event_type = ZPO_ICMP_ECHO_REPLY_EVENT_UID;
                } else if(hdr.icmp.type_ == ICMP_ECHO) {
                    meta.event_type = ZPO_ICMP_ECHO_REQUEST_EVENT_UID;
                }
            }
        }
    }
...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) {
    ...
    action construct_eth_event_header() {
        // construct rna_t header
        // rna_version = ....
        // rna_type = RNA_TYPE_ETH
    ...
    apply {
        ...
        // EVENT PACKETS
            ...
            if(meta.event_type == ZPO_NO_EVENT_UID) {
                ...
                if (meta.event_type == ZPO_ARP_REPLY_EVENT_UID) {
                ...
                } else if (meta.event_type == ZPO_ARP_REQUEST_EVENT_UID) {
                    ...
                    hdr.arp.setInvalid();
                    hdr.arp_ipv4.setInvalid();
                } else if (meta.event_type == ZPO_ICMP_ECHO_REPLY_EVENT_UID) {
                    ...
                    // hdr.icmp_echo_reply_event.v6 = (z_bool) 0;

                    // The ICMP type of the current packet.
                    hdr.icmp_echo_reply_event.itype = (z_count) hdr.icmp.type_;

                    // The ICMP code of the current packet.
                    hdr.icmp_echo_reply_event.icode = (z_count) hdr.icmp.code;

                    // The length of the ICMP payload. (total ipv4 length - (ipv4 header + icmp header))
                    // hdr.icmp_echo_reply_event.len = (z_count) hdr.ipv4.total_len - hdr.ipv4.minSizeInBytes() - hdr.icmp.minSizeInBytes();
                    ...
                    if(hdr.ipv6.isValid()) {
                        ...
                        hdr.icmp_echo_reply_event.len = (z_count) hdr.ipv6.payload_length - hdr.icmp.minSizeInBytes();
                        hdr.icmp_echo_reply_event.ttl = (z_count) hdr.ipv6.hop_limit;
                    ...
                    hdr.icmp.setInvalid();
                } else if (meta.event_type == ZPO_ICMP_ECHO_REQUEST_EVENT_UID) {
